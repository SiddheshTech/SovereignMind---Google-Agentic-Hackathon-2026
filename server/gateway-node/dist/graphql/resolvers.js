...
const AuthorityProposal_1 = require("../models/AuthorityProposal");
const CrisisSimulation_1 = require("../models/CrisisSimulation");
const CivilizationGenome_1 = require("../models/CivilizationGenome");
const RiskRadar_1 = require("../models/RiskRadar");
const Forecast_1 = require("../models/Forecast");
const BlackSwan_1 = require("../models/BlackSwan");
const NationModel_1 = require("../models/NationModel");
const DependenciesGraph_1 = require("../models/DependenciesGraph");
const InfrastructureData_1 = require("../models/InfrastructureData");
const AnalyticsData_1 = require("../models/AnalyticsData");
const CrisisData_1 = require("../models/CrisisData");
const ProcurementData_1 = require("../models/ProcurementData");
const IntelligenceData_1 = require("../models/IntelligenceData");
const CopilotData_1 = __importDefault(require("../models/CopilotData"));
const CollaborationData_1 = require("../models/CollaborationData");
const CommandCenterData_1 = require("../models/CommandCenterData");
const ExecutiveBriefingData_1 = require("../models/ExecutiveBriefingData");
const db_1 = __importDefault(require("../config/db"));
(0, db_1.default)();
/** Helper to convert gRPC CrisisScenarioResponse to GraphQL shape */
function formatSimulation(doc) {
    ...
    return {
        ...
        simulatedPopulation: {
            ...
            ageGroups: (pop.ageGroups || []).map((a) => ({
                group: a.group,
                percentage: a.percentage,
                reaction: a.reaction,
            })),
            incomeClasses: (pop.incomeClasses || []).map((c) => ({
            ...
            politicalPreferences: (pop.politicalPreferences || []).map((p) => ({
                faction: p.faction,
...
function grpcScenarioToDoc(res) {
    ...
    return {
        ...
        simulatedPopulation: {
            ...
            ageGroups: (pop.age_groups || []).map((a) => ({
                group: a.group,
                percentage: a.percentage,
                reaction: a.reaction,
            })),
            incomeClasses: (pop.income_classes || []).map((c) => ({
            ...
            politicalPreferences: (res.political_preferences || []).map((p) => ({
                faction: p.faction,
                percentage: p.percentage,
                sentiment: p.sentiment,
            })),
...
exports.resolvers = {
    Query: {
        ...
        getDetailedGenome: async (_, { countryCode }) => {
            try {
                const genome = await CivilizationGenome_1.CivilizationGenome.findOne({ countryCode: countryCode.toUpperCase() });
                return genome;
            }
            catch (err) {
        ...
        getSimilarity: async (_, { countryCode }) => {
            try {
                const response = await (0, client_1.calculateSimilarity)(countryCode);
                return response.results || [];
            }
            catch (err) {
        ...
        getAuthorityProposals: async () => {
            try {
                const proposals = await AuthorityProposal_1.AuthorityProposal.find().sort({ createdAt: -1 });
        ...
        // ── Sandbox Queries ────────────────────────────────────────────────────────
        getCrisisSimulations: async (_, { limit }) => {
            try {
                const sims = await CrisisSimulation_1.CrisisSimulation.find().sort({ createdAt: -1 }).limit(limit || 20);
        ...
        getCrisisSimulation: async (_, { id }) => {
            try {
                const sim = await CrisisSimulation_1.CrisisSimulation.findById(id);
                if (!sim)
        ...
        getSystemSettings: async () => {
            try {
                const s = await (0, client_1.getSystemSettings)();
                return {
        ...
        getSecurityClearances: async () => {
            try {
                const res = await (0, client_1.getSecurityClearances)();
                return (res.clearances || []).map((c) => ({
                    id: c.id,
                    name: c.name,
        ...
        getAccessTokens: async () => {
            try {
                const res = await (0, client_1.getAccessTokens)();
        ...
        getAlertRules: async () => {
            try {
                const res = await (0, client_1.getAlertRules)();
                return res?.rules || [];
            }
            catch (err) {
        ...
        getComplianceRecords: async () => {
            try {
                const res = await (0, client_1.getComplianceRecords)();
        ...
        getRiskRadarData: async () => {
            try {
                const threats = await RiskRadar_1.RiskRadarThreat.find().sort({ probability: -1 });
                const signals = await RiskRadar_1.EarlyWarningSignal.find().sort({ createdAt: -1 });
                return {
        ...
        getForecastData: async () => {
            try {
                const forecast = await Forecast_1.Forecast.findOne().sort({ createdAt: -1 });
                if (!forecast)
                    return null;
                return {
        ...
        getBlackSwanData: async () => {
            try {
                const bs = await BlackSwan_1.BlackSwan.findOne().sort({ createdAt: -1 });
                if (!bs)
                    return null;
                return {
                    id: bs._id.toString(),
        ...
        getNationModelData: async () => {
            try {
                ...
                let model = await NationModel_1.NationModel.findOne().sort({ updatedAt: -1 });
                if (!model) {
                    console.log('🔄 [GraphQL] Fetching initial Nation Model from gRPC');
                    const grpcRes = await (0, client_1.getNationModelData)();
                    model = await NationModel_1.NationModel.create({
        ...
        getDependenciesData: async () => {
            try {
                const graph = await DependenciesGraph_1.DependenciesGraph.findOne().sort({ createdAt: -1 });
                if (!graph)
                    return null;
                return {
        ...
        getInfrastructureData: async () => {
            try {
                const data = await InfrastructureData_1.InfrastructureData.findOne().sort({ createdAt: -1 });
                if (!data)
                    return null;
                return {
        ...
        getAnalyticsData: async () => {
            try {
                let data = await AnalyticsData_1.AnalyticsData.findOne().sort({ createdAt: -1 });
                if (!data) {
                    console.log('🔄 [GraphQL] Fetching initial Analytics from gRPC');
                    const grpcRes = await (0, client_1.getAnalyticsData)();
                    data = await AnalyticsData_1.AnalyticsData.create({
        ...
        getCrisisData: async () => {
            try {
                const latest = await CrisisData_1.CrisisData.findOne().sort({ createdAt: -1 });
                if (latest) {
                    return {
                ...
                const grpcRes = await (0, client_1.getCrisisData)();
                const incidents = grpcRes.incidents.map((i) => ({
                    id: i.id, title: i.title, time: i.time, desc: i.desc, severity: i.severity, status: i.status, responders: i.responders
                }));
                const mapNodes = grpcRes.mapNodes.map((m) => ({
                    id: m.id, x: m.x, y: m.y, type: m.type, status: m.status
                }));
                const channels = grpcRes.channels.map((c) => ({
                    name: c.name, active: c.active, status: c.status
                }));
                const logs = grpcRes.logs.map((l) => ({
                    time: l.time, msg: l.msg, type: l.type
                }));
                const policyOptions = grpcRes.policyOptions.map((p) => ({
                    title: p.title, desc: p.desc, tag: p.tag
                }));
                const comparatorRows = grpcRes.comparatorRows.map((r) => ({
                    metric: r.metric, optA: r.optA, optB: r.optB, optC: r.optC
                }));
                const chatMsgs = grpcRes.chatMsgs.map((c) => ({
                    name: c.name, msg: c.msg, time: c.time, isAI: c.isAI
                }));
                const data = await CrisisData_1.CrisisData.create({
        ...
        getProcurementData: async () => {
            try {
                ...
                const existingData = await ProcurementData_1.ProcurementData.findOne().sort({ updatedAt: -1 });
                if (existingData)
                    return {
                ...
                const grpcRes = await (0, client_1.getProcurementData)();
                const data = await ProcurementData_1.ProcurementData.create({
        ...
        getIntelligenceData: async () => {
            try {
                ...
                const existingData = await IntelligenceData_1.IntelligenceData.findOne().sort({ updatedAt: -1 });
                if (existingData)
                    return {
                ...
                const grpcRes = await (0, client_1.getIntelligenceGraphData)();
                const data = await IntelligenceData_1.IntelligenceData.create({
                    nodes: grpcRes.nodes || [],
                    edges: grpcRes.edges || [],
        ...
        getCopilotSession: async (_, { sessionId }) => {
            let session = await CopilotData_1.default.findOne({ sessionId });
            if (!session) {
                session = new CopilotData_1.default({
                    sessionId,
                    messages: [{
                            id: Date.now(),
                            role: 'assistant',
        ...
        getCollaborationData: async () => {
            try {
                ...
                let localData = await CollaborationData_1.CollaborationData.findOne().sort({ createdAt: -1 });
                if (!localData) {
                    const grpcRes = await (0, client_1.getCollaborationData)();
                    localData = await CollaborationData_1.CollaborationData.create({
                        rooms: grpcRes.rooms || [],
                        users: grpcRes.users || [],
        ...
        getCommandCenterData: async () => {
            try {
                ...
                let localData = await CommandCenterData_1.CommandCenterSnapshot.findOne().sort({ createdAt: -1 });
                if (!localData) {
                    const grpcRes = await (0, client_1.getCommandCenterData)();
                    localData = await CommandCenterData_1.CommandCenterSnapshot.create(grpcRes);
                }
                return {
        ...
        getExecutiveBriefingData: async () => {
            try {
                ...
                let localData = await ExecutiveBriefingData_1.ExecutiveBriefingSnapshot.findOne().sort({ createdAt: -1 });
                if (!localData) {
                    const grpcRes = await (0, client_1.getExecutiveBriefingData)();
                    localData = await ExecutiveBriefingData_1.ExecutiveBriefingSnapshot.create(grpcRes);
                }
                return {
        ...
        getMetricDetail: async (_, { metricId }) => {
            try {
                console.log(`📡 [GraphQL] getMetricDetail called for: ${metricId}`);
                const grpcRes = await (0, client_1.getMetricDetail)(metricId);
                return grpcRes;
            }
            catch (err) {
        ...
        getOperatorDashboardData: async () => {
            try {
                ...
                const grpcRes = await (0, client_1.getOperatorDashboardData)();
                return grpcRes;
            }
            catch (err) {
    ...
    Mutation: {
        ...
        validateAuthorityProposal: async (_, { title }) => {
            try {
                const res = await (0, client_1.evaluateAuthorityProposal)(title);
                const newProposal = new AuthorityProposal_1.AuthorityProposal({
                    title: res.title,
                ...
                (0, simulation_stream_1.broadcast)({ type: 'AUTHORITY_PROPOSAL_ADDED', data: responseData }, '/ws/authority-maps');
                return responseData;
            }
            catch (err) {
        ...
        launchCrisisSimulation: async (_, { crises, scenarioName }) => {
            try {
                ...
                const grpcRes = await (0, client_1.runCrisisScenario)(crises, name);
                const docData = grpcScenarioToDoc(grpcRes);
                // 2. Save to MongoDB
                const saved = await CrisisSimulation_1.CrisisSimulation.create(docData);
                const simId = saved._id.toString();
                // 3. Trigger real-time tick stream for Simulations tab (non-blocking)
                (0, simulation_stream_1.triggerDetailedSimStream)(crises, 12, simId);
                // 4. Broadcast to all sandbox WS clients
                (0, simulation_stream_1.broadcast)({
        ...
        generateRecoveryPaths: async (_, { simulationId, crises }) => {
            try {
                console.log(`🔮 [GraphQL] generateRecoveryPaths for sim ${simulationId}`);
                // gRPC → FastAPI → Gemini
                const grpcRes = await (0, client_1.generateRecoveryPaths)(crises, simulationId);
                const recoveryDoc = {
                    bestCase: {
                ...
                await CrisisSimulation_1.CrisisSimulation.findByIdAndUpdate(simulationId, {
        ...
        saveScenario: async (_, { crises, scenarioName }) => {
            try {
                console.log(`💾 [GraphQL] saveScenario: "${scenarioName}" with crises: ${crises}`);
                // Run AI analysis for the saved scenario
                const grpcRes = await (0, client_1.runCrisisScenario)(crises, scenarioName);
                const docData = grpcScenarioToDoc(grpcRes);
                const saved = await CrisisSimulation_1.CrisisSimulation.create(docData);
                (0, simulation_stream_1.broadcast)({
                    type: 'SCENARIO_SAVED',
        ...
        // ── Settings Mutations ────────────────────────────────────────────────────
        saveSystemSettings: async (_, { input }) => {
            try {
                // Map camelCase GraphQL input to snake_case for gRPC
                const updates = {};
                if (input.operatorName !== undefined)
                    updates.operator_name = input.operatorName;
                if (input.operatorId !== undefined)
                    updates.operator_id = input.operatorId;
                if (input.operatorInstitution !== undefined)
                    updates.operator_institution = input.operatorInstitution;
                if (input.operatorRole !== undefined)
                    updates.operator_role = input.operatorRole;
                if (input.operatorTogglesJson !== undefined)
                    updates.operator_toggles_json = input.operatorTogglesJson;
                if (input.modelProcessingBound !== undefined)
                    updates.model_processing_bound = input.modelProcessingBound;
                if (input.clearanceMatrixJson !== undefined)
                    updates.clearance_matrix_json = input.clearanceMatrixJson;
                if (input.activeRegion !== undefined)
                    updates.active_region = input.activeRegion;
                if (input.storagePoliciesJson !== undefined)
                    updates.storage_policies_json = input.storagePoliciesJson;
                if (input.processingBoundary !== undefined)
                    updates.processing_boundary = input.processingBoundary;
                if (input.theme !== undefined)
                    updates.theme = input.theme;
                if (input.telemetryTogglesJson !== undefined)
                    updates.telemetry_toggles_json = input.telemetryTogglesJson;
                if (input.notificationChannelsJson !== undefined)
                    updates.notification_channels_json = input.notificationChannelsJson;
                if (input.networkProtocolsJson !== undefined)
                    updates.network_protocols_json = input.networkProtocolsJson;
                if (input.networkPoliciesJson !== undefined)
                    updates.network_policies_json = input.networkPoliciesJson;
                const s = await (0, client_1.saveSystemSettings)(updates);
                const result = {
                    operatorName: s.operator_name,
                    operatorId: s.operator_id,
                    operatorInstitution: s.operator_institution,
                ...
                (0, simulation_stream_1.broadcast)({ type: 'SETTINGS_UPDATED', data: result }, '/ws/settings');
                return result;
            }
            catch (err) {
        ...
        updateSecurityClearance: async (_, { id, level, status }) => {
            try {
                const c = await (0, client_1.updateSecurityClearance)(id, level, status);
                const result = {
                    id: c.id, name: c.name, serviceId: c.service_id,
                    level: c.level, status: c.status, expiry: c.expiry,
                };
                (0, simulation_stream_1.broadcast)({ type: 'CLEARANCE_UPDATED', data: result }, '/ws/settings');
                return result;
            }
            catch (err) {
        ...
        generateAccessToken: async (_, { tokenType, environment, permissions, owner }) => {
            try {
                const t = await (0, client_1.generateAccessToken)(tokenType, environment, permissions, owner);
                const result = {
                    id: t.id, owner: t.owner, tokenType: t.token_type,
                    created: t.created, lastUsed: t.last_used, status: t.status,
                };
                (0, simulation_stream_1.broadcast)({ type: 'TOKEN_GENERATED', data: result }, '/ws/settings');
                return result;
            }
            catch (err) {
        ...
        updateAccessToken: async (_, { id, action }) => {
            try {
                const t = await (0, client_1.updateAccessToken)(id, action);
                const result = {
                    id: t.id, owner: t.owner, tokenType: t.token_type,
                    created: t.created, lastUsed: t.last_used, status: t.status,
                };
                (0, simulation_stream_1.broadcast)({ type: 'TOKEN_UPDATED', data: result }, '/ws/settings');
                return result;
            }
            catch (err) {
        ...
        saveAlertRule: async (_, args) => {
            try {
                const r = await (0, client_1.saveAlertRule)({
                    id: args.id,
                    name: args.name,
                ...
                (0, simulation_stream_1.broadcast)({ type: 'ALERT_RULE_SAVED', data: result }, '/ws/settings');
                return result;
            }
            catch (err) {
        ...
        deleteAlertRule: async (_, { id }) => {
            try {
                const res = await (0, client_1.deleteAlertRule)(id);
                (0, simulation_stream_1.broadcast)({ type: 'ALERT_RULE_DELETED', id }, '/ws/settings');
                return res;
            }
            catch (err) {
        ...
        saveComplianceRecord: async (_, { input }) => {
            try {
                const args = {
                    id: input.id || '',
                    name: input.name,
                ...
                const record = await (0, client_1.saveComplianceRecord)(args);
                const mapped = { ...record, lastAudit: record.last_audit };
                (0, simulation_stream_1.broadcast)({ type: 'COMPLIANCE_RECORD_SAVED', record: mapped }, '/ws/settings');
                return mapped;
            }
            catch (err) {
        ...
        deleteComplianceRecord: async (_, { id }) => {
            try {
                const res = await (0, client_1.deleteComplianceRecord)(id);
                (0, simulation_stream_1.broadcast)({ type: 'COMPLIANCE_RECORD_DELETED', id }, '/ws/settings');
                return res;
            }
            catch (err) {
        ...
        generateRiskRadarData: async () => {
            try {
                ...
                const grpcRes = await (0, client_1.generateRiskRadarData)();
                // Clear existing mock data
                await RiskRadar_1.RiskRadarThreat.deleteMany({});
                await RiskRadar_1.EarlyWarningSignal.deleteMany({});
                // Insert new generated data from gRPC (FastAPI)
                const newThreats = grpcRes.threats || [];
                const newSignals = grpcRes.signals || [];
                const threatDocs = await RiskRadar_1.RiskRadarThreat.insertMany(newThreats.map((t) => ({
                    threatName: t.threat_name,
                    probability: t.probability,
                    severity: t.severity,
                    timeToImpact: t.time_to_impact,
                })));
                const signalDocs = await RiskRadar_1.EarlyWarningSignal.insertMany(newSignals.map((s) => ({
                ...
                (0, simulation_stream_1.broadcast)({ type: 'RISK_RADAR_DATA_UPDATED', data: result }, '/ws/risk-radar');
                return result;
            }
            catch (err) {
        ...
        generateForecastData: async () => {
            try {
                ...
                const grpcRes = await (0, client_1.generateForecastData)('10_years');
                await Forecast_1.Forecast.deleteMany({});
                const newForecast = await Forecast_1.Forecast.create({
                    timelinePoints: grpcRes.timeline_points.map((tp) => ({
                        year: tp.year,
                        title: tp.title,
                ...
                (0, simulation_stream_1.broadcast)({ type: 'FORECAST_DATA_UPDATED', data: result }, '/ws/forecasting');
                return result;
            }
            catch (err) {
        ...
        generateBlackSwanData: async () => {
            try {
                ...
                const grpcRes = await (0, client_1.generateBlackSwanData)();
                await BlackSwan_1.BlackSwan.deleteMany({});
                const newBs = await BlackSwan_1.BlackSwan.create({
                    anomalies: grpcRes.anomalies.map((a) => ({
                        title: a.title,
                ...
                (0, simulation_stream_1.broadcast)({ type: 'BLACK_SWAN_DATA_UPDATED', data: result }, '/ws/black-swan');
                return result;
            }
            catch (err) {
        ...
        executeShockScenario: async (_, { shockName }) => {
            try {
                console.log(`⚡ [GraphQL] executeShockScenario called: ${shockName}`);
                const grpcRes = await (0, client_1.executeShockScenario)(shockName);
                const updatedModel = await NationModel_1.NationModel.create({
                    economyVal: grpcRes.economy_val,
                    economyTrend: grpcRes.economy_trend,
                    societyVal: grpcRes.society_val,
                    societySubtitle: grpcRes.society_subtitle,
                ...
                (0, simulation_stream_1.broadcast)({ type: 'NATION_MODEL_UPDATED', data: result }, '/ws/nation-model');
                return result;
            }
            catch (err) {
        ...
        generateDependenciesData: async () => {
            try {
                ...
                const grpcRes = await (0, client_1.generateDependenciesData)();
                const nodes = grpcRes.nodes.map((n) => ({
                    id: n.id,
                    title: n.title,
                    status: n.status,
                    icon: n.icon
                }));
                const edges = grpcRes.edges.map((e) => ({
                    fromId: e.from_id,
                    toId: e.to_id,
                    status: e.status
                }));
                const graph = await DependenciesGraph_1.DependenciesGraph.create({ nodes, edges });
                const result = {
                    id: graph._id.toString(),
                    nodes: graph.nodes,
                    edges: graph.edges,
                ...
                (0, simulation_stream_1.broadcast)({ type: 'DEPENDENCIES_DATA_UPDATED', data: result }, '/ws/dependencies');
                return result;
            }
            catch (err) {
        ...
        simulateInfrastructureUpdate: async () => {
            try {
                ...
                const grpcRes = await (0, client_1.simulateInfrastructureUpdate)();
                const nodes = grpcRes.nodes.map((n) => ({
                    id: n.id,
                    title: n.title,
                    icon: n.icon,
                    ...
                    load: n.load,
                    metrics: n.metrics.map((m) => ({ label: m.label, val: m.val }))
                }));
                const data = await InfrastructureData_1.InfrastructureData.create({ nodes });
                const result = {
                    id: data._id.toString(),
                    nodes: data.nodes,
                    createdAt: data.createdAt.toISOString()
                };
                (0, simulation_stream_1.broadcast)({ type: 'INFRASTRUCTURE_DATA_UPDATED', data: result }, '/ws/infrastructure');
                return result;
            }
            catch (err) {
        ...
        simulateAnalyticsUpdate: async () => {
            try {
                ...
                const grpcRes = await (0, client_1.simulateAnalyticsUpdate)();
                const keyMetrics = grpcRes.keyMetrics.map((m) => ({
                    title: m.title, value: m.value, trend: m.trend, trendDir: m.trendDir, statusColor: m.statusColor
                }));
                const vectorData = grpcRes.vectorData.map((v) => ({
                    value: v.value, isAnomaly: v.isAnomaly
                }));
                const scoring = grpcRes.scoring.map((s) => ({
                    label: s.label, score: s.score, baseline: s.baseline, color: s.color
                }));
                const reports = grpcRes.reports.map((r) => ({
                    id: r.id, title: r.title, desc: r.desc, date: r.date, confidence: r.confidence, category: r.category, isCritical: r.isCritical
                }));
                const data = await AnalyticsData_1.AnalyticsData.create({ keyMetrics, vectorData, scoring, reports });
                const result = {
                    id: data._id.toString(),
                    keyMetrics: data.keyMetrics,
                    vectorData: data.vectorData,
                ...
                (0, simulation_stream_1.broadcast)({ type: 'ANALYTICS_DATA_UPDATED', data: result }, '/ws/analytics');
                return result;
            }
            catch (err) {
        ...
        generateSynthesisReport: async (_, { domain, horizon }) => {
            try {
                console.log('⚡ [GraphQL] generateSynthesisReport called');
                const reportGrpc = await (0, client_1.generateSynthesisReport)(domain, horizon);
                // Fetch current analytics data
                const currentData = await AnalyticsData_1.AnalyticsData.findOne().sort({ createdAt: -1 });
        ...
        simulateCrisisUpdate: async () => {
            try {
                const grpcRes = await (0, client_1.simulateCrisisUpdate)();
                const incidents = grpcRes.incidents.map((i) => ({
                    id: i.id, title: i.title, time: i.time, desc: i.desc, severity: i.severity, status: i.status, responders: i.responders
                }));
                const mapNodes = grpcRes.mapNodes.map((m) => ({
                ...
                (0, simulation_stream_1.broadcast)({ type: 'CRISIS_DATA_UPDATED', data: result }, '/ws/crisis');
                return result;
            }
            catch (err) {
        ...
        generatePolicyOptions: async (_, { prompt }) => {
            try {
                console.log(`⚡ [GraphQL] generatePolicyOptions called prompt=${prompt}`);
                const policyGrpc = await (0, client_1.generatePolicyOptions)(prompt);
                // Fetch current crisis data to merge with new policy options
                const currentData = await CrisisData_1.CrisisData.findOne().sort({ createdAt: -1 });
                ...
                const options = policyGrpc.options.map((p) => ({ title: p.title, desc: p.desc, tag: p.tag }));
                const rows = policyGrpc.rows.map((r) => ({ metric: r.metric, optA: r.optA, optB: r.optB, optC: r.optC }));
                const data = await CrisisData_1.CrisisData.create({
        ...
        simulateProcurementUpdate: async () => {
            try {
                ...
                const grpcRes = await (0, client_1.simulateProcurementUpdate)();
                const data = await ProcurementData_1.ProcurementData.create({
                    vendors: grpcRes.vendors || [],
                    contracts: grpcRes.contracts || [],
                    mapNodes: grpcRes.mapNodes || [],
                ...
                (0, simulation_stream_1.broadcast)({ type: 'PROCUREMENT_DATA_UPDATED', data: result }, '/ws/procurement');
                return result;
            }
            catch (err) {
        ...
        simulateIntelligenceUpdate: async () => {
            try {
                ...
                const grpcRes = await (0, client_1.simulateIntelligenceUpdate)();
                await IntelligenceData_1.IntelligenceData.deleteMany({});
                const data = await IntelligenceData_1.IntelligenceData.create({
                    nodes: grpcRes.nodes || [],
                    edges: grpcRes.edges || [],
                    topologyStats: grpcRes.topologyStats || {
        ...
        sendCopilotMessage: async (_, { sessionId, prompt }) => {
            let session = await CopilotData_1.default.findOne({ sessionId });
            if (!session) {
                session = new CopilotData_1.default({ sessionId, messages: [] });
            }
            const userMsg = { id: Date.now(), role: 'user', content: prompt, timestamp: new Date() };
            session.messages.push(userMsg);
            await session.save();
            const response = await (0, client_1.sendCopilotMessage)(prompt);
            const assistantMsg = { id: Date.now() + 1, role: 'assistant', content: response.content, timestamp: new Date() };
            session.messages.push(assistantMsg);
            await session.save();
            return assistantMsg;
        },
        clearCopilotSession: async (_, { sessionId }) => {
            await CopilotData_1.default.deleteOne({ sessionId });
            return true;
        },
        createCollaborationRoom: async (_, { name, type, category }) => {
            try {
                const grpcRes = await (0, client_1.createCollaborationRoom)(name, type, category);
                let localData = await CollaborationData_1.CollaborationData.findOne().sort({ createdAt: -1 });
                if (localData) {
        ...
        sendCollaborationMessage: async (_, { roomId, sender, text }) => {
            try {
                const grpcRes = await (0, client_1.sendCollaborationMessage)(roomId, sender, text);
                let localData = await CollaborationData_1.CollaborationData.findOne().sort({ createdAt: -1 });
                if (localData) {
        ...
        simulateEmergencyPowers: async (_, { scenario }) => {
            try {
                console.log("⚡ [GraphQL] simulateEmergencyPowers triggered");
                const grpcRes = await (0, client_1.simulateEmergencyPowers)(scenario);
                return grpcRes;
            }
            catch (err) {
        ...
        analyzeTreatyConstraints: async (_, { proposal }) => {
            try {
                console.log("⚡ [GraphQL] analyzeTreatyConstraints triggered");
                const grpcRes = await (0, client_1.analyzeTreatyConstraints)(proposal);
                return grpcRes;
            }
            catch (err) {
        ...
        refreshCommandCenterData: async () => {
            try {
                ...
                const grpcRes = await (0, client_1.refreshCommandCenterData)();
                const localData = await CommandCenterData_1.CommandCenterSnapshot.create(grpcRes);
                (0, simulation_stream_1.broadcast)({
        ...
        refreshExecutiveBriefingData: async () => {
            try {
                ...
                const grpcRes = await (0, client_1.refreshExecutiveBriefingData)();
                const localData = await ExecutiveBriefingData_1.ExecutiveBriefingSnapshot.create(grpcRes);
                (0, simulation_stream_1.broadcast)({
