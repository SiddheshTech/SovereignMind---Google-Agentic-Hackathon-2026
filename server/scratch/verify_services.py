...
from app.services.training_engine import model_training_engine  # noqa: E402
from app.agents.mcp_server import mcp_server  # noqa: E402
from app.tasks.tasks import async_constitutional_audit  # noqa: E402
from app.db.kafka_client import kafka_client  # noqa: E402
from app.agents.llm_router import llm_router  # noqa: E402
from app.db.models import GenomeModel, ConstitutionalAuditModel  # noqa: E402
from app.db.database import init_db, AsyncSessionLocal  # noqa: E402
from app.eval.optimizer import prompt_optimizer  # noqa: E402
from app.services.sandbox_engine import sandbox_engine  # noqa: E402
from app.services.genome_engine import genome_engine  # noqa: E402
...
async def run_diagnostics():
    ...
    async with AsyncSessionLocal() as session:
        ...
        # Clean up existing test rows to guarantee unique constraints are met
        # on consecutive runs!
        from sqlmodel import select
        existing_result = await session.execute(
            select(GenomeModel).where(GenomeModel.country_code == "US")
        )
        ...
        try:
            ...
            await session.commit()
            print("✅ SQLModel Genome successfully saved and "
                  "committed in SQLite/PostgreSQL!")
        except Exception:
        ...
        firestore_client.collection("civilizations").document(
            "US").set({"status": "resilient"})
        print("✅ Firebase Integration successfully verified!")

        # Verify MongoDB Integration
        from app.db.mongodb_client import mongo_client
        mongo_client["sovereignmind"]["civilizations"].insert_one(
        ...
        assert decoded.get(
            "sub") == "admin", "OAuth 2.0 Decoded subject mismatch!"
    ...
    for provider in ["gemini", "groq", "mistral"]:
        resp = await llm_router.generate_response(
            ...
            user_prompt=("Audit proposed action: nationwide curfew "
                         "during food scarcity.")
        )
        print(f"✅ Provider '{provider}' routed successfully. "
              f"Response length: {len(resp)} characters.")
    ...
    print(f"✅ MCP tools listed successfully: {names}")
    ...
    print("✅ MCP tool 'audit_policy_proposal' executed "
          "successfully via JSON-RPC!")
    ...
    name = us_gen['nation_name']
    resil = us_gen['overall_resilience_index']
    print(f"✅ Genome found: {name} (Resilience: {resil})")

    # 7. Test PyTorch Simulation Sandbox
    print("\n🎬 [Test 7] PyTorch Neural Grid Cascade Simulation "
          "Verification...")
    ticks = []
    events = ["grid_failure", "currency_collapse"]
    async for tick in sandbox_engine.run_simulation("US", 3, events):
        ticks.append(tick)
        ep = tick['epoch']
        panic = tick['population']['panic_level']
        prob = tick['instability']['systemic_collapse_probability']
        print(f"   Epoch {ep}: Panic={panic:.3f}, Collapse Prob={prob:.4f}")
    ...
    task_desc = ("Coordinate communications under grid failure "
                 "without violating rights.")
    original_prompt = ("You are an automated logistics agent in charge of "
                       "securing local cell sites during emergencies.")

    opt_res = await prompt_optimizer.optimize_prompt(
        "Logistics-Agent", task_desc, original_prompt
    )
    gain = opt_res['performance_gain']
    print(f"✅ Prompt Optimization Complete. Performance Gain: +{gain:.1f}%")
    ...
    print("✅ Deep Learning stability model successfully trained!")
    acc = train_res['final_accuracy'] * 100
    print(f"   Final Accuracy: {acc:.1f}% (Calibrated: 90-95% Target)")
    print(f"   Weights saved to: {train_res['weights_path']}")

    # 10. Test Gemini Agent Self-Introspection & Autonomous Rewrite Loop
    print("\n🕵️‍♂️ [Test 10] Gemini Agent Self-Introspection & "
          "Autonomous Rewrite Loop...")

    # A. Seed an initial unconstitutional infraction trace
    print("   - Seeding past Fourth Amendment infraction trace to "
          "database and MCP local cache...")
    try:
        async with AsyncSessionLocal() as session:
            infraction_record = ConstitutionalAuditModel(
                country_code="US",
                proposed_action=("Direct federal mandate to confiscate local "
                                 "citizen cell sites and private towers"),
                context="Extreme civil unrest under grid failure",
                is_authorized=False,
                infraction_risk_score=75.0,
                alternate_recommendation=("DENIED: Proposing physical "
                                          "seizure violating Fourth "
                                          "Amendment protections. "
                                          "Switch to leasing contracts.")
            )
    ...
    audit_proposal = ("Draft emergency operational plan to secure local "
                      "wireless communications hubs.")
    final_decision = (
        await constitutional_adjudicator.self_introspect_and_execute(
            "US", audit_proposal
        )
    )
    ...
    print("✅ Prompt rewritten dynamically to guarantee Fourth "
          "Amendment compliance.")
    print("✅ Autonomous self-correction loop validated and completed!")

    print("\n" + "=" * 60)
    print("🎉 Deep-dive operational tests completed. "
          "100% Backend Topologies Active.")
    print("=" * 60)
